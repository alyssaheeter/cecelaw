...
def should_use_mock_on_failure(request_obj: Dict) -> bool:
    return is_truthy(request_obj.get("use_mock_on_api_error", False)) or is_truthy(
        os.getenv("COPYGEN_ALLOW_MOCK_ON_API_FAILURE", "false")
    )
...
def generate_raw_output(
    ...
    allow_no_key_mock = is_truthy(request_obj.get("use_mock_when_no_api_key", False))

    if not has_api_key:
        if allow_no_key_mock:
            print("INFO: GEMINI_API_KEY not set; using mock output.")
            return build_mock_response(request_obj)
        raise RuntimeError("GEMINI_API_KEY is not set and mock mode is disabled.")

    try:
        return call_gemini(system_instruction, user_prompt, model, temperature)
    except Exception as exc:  # noqa: BLE001
        if should_use_mock_on_failure(request_obj) and is_quota_or_billing_error(exc):
            print(f"WARNING: Gemini API quota/billing error detected ({exc}). Falling back to mock output.")
            return build_mock_response(request_obj)
        raise
...
def main() -> int:
    ...
    try:
        raw = generate_raw_output(request_obj, system_instruction, user_prompt, model, temperature)
